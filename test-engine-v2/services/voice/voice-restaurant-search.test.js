...
/**
 * Voice — Restaurant Search Flow
 *
 * Validates the menu search flow in a single session:
 *   1. search_menu_item → returns matching items for a query
...
const CALL_ID = uniqueVoiceCallId('test-call-restaurant-search');

// ─────────────────────────────────────────────────────────────────────────────

describe('voice / restaurant / search', () => {
  let internalCallId;
  ...
  // ── Step 1: search_menu_item ───────────────────────────────────────────────

  it('step 1 — search_menu_item returns matching items for query', async () => {
    ...
    expect(toolResult.query).toBe('salami');
    expect(Array.isArray(toolResult.items)).toBe(true);
    expect(toolResult.items.length).toBeGreaterThan(0);

    const firstItem = toolResult.items[0];
