...
suite.test('Setup — create customer for RLS tests', async (ctx) => {
  const res = await tenantA.post('/customer', {
    name:  'RLS Test Customer',
    email: 'rls.test.customer@example.com',
    phone: '+49000000002',
  });
...
suite.test('Cleanup — RLS test customer', async (ctx) => {
  if (!ctx.rlsCustomerId) return;
  await tenantA.delete(`/customer/${ctx.rlsCustomerId}`);
});
