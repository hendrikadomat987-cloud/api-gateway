...
// ═════════════════════════════════════════════════════════════════════════════
// SETUP — create a real customer to satisfy the FK constraint
// ═════════════════════════════════════════════════════════════════════════════

suite.test('Setup — create customer for request tests', async (ctx) => {
  const res = await client.post('/customer', {
    name:  'Request Test Customer',
    email: 'request.test.customer@example.com',
    phone: '+49000000001',
  });
...
}, { critical: true }); // no customer → all creates would fail FK constraint
...
suite.test('POST request — missing type → VALIDATION_ERROR', async (ctx) => {
  const res = await client.post('/requests', {
    customer_id: ctx.customerId,
  });
...
suite.test('POST request — invalid type → VALIDATION_ERROR', async (ctx) => {
  const res = await client.post('/requests', {
    customer_id: ctx.customerId,
    type:        'unknown_type',
  });
...
suite.test('DELETE request — already deleted → idempotent success', async (ctx) => {
...
suite.test('Cleanup — setup customer', async (ctx) => {
  if (!ctx.customerId) return;
  await client.delete(`/customer/${ctx.customerId}`);
});
